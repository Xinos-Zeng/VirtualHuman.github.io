...
### 2.3 支持上传json并解析模拟
1. 请在主页的左边新增一个可展开收起的栏位，该栏位支持用户上传一份和agent_logs_structure.md数据结构相同的json文件，读取该文件后使用agentLogsParser.js解析。但是注意上传的json不存在"node_meta"这个模拟使用的字段。需要考虑兼容性。
2. 将原来的"开始模拟"按钮移到左边栏位并改名成"使用模拟数据模拟"，此时会走目前的这套模拟逻辑流程。
3. 左边栏位新增一个"使用上传数据模拟"，该按钮和"使用模拟数据模拟"垂直方向排列，未上传文件时不可点击。
